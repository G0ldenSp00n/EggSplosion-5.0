...
@NullMarked
public class TeamArgument implements CustomArgumentType.Converted<Teams, String> {
  ...
  @Override
  public Teams convert(String nativeType) throws CommandSyntaxException {
    return Teams.fromString(nativeType);
  }

  @Override
  public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> ctx, SuggestionsBuilder builder) {
    Arrays.stream(Teams.values()).map(entry -> entry.asString())
        .filter(entry -> entry
            .toLowerCase()
            .startsWith(builder
